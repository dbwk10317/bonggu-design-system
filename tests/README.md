...
- release-regressions.cjs: 패키지가 npmjs 공개(`publishConfig.access: public`, 레지스트리 미지정)이고 React peer 범위가 게이트 조합과 같은지, 발행 경로가 태그 워크플로 하나뿐이고 게이트 뒤에 `NPM_TOKEN`으로 publish하며 prerelease는 `next`·정식은 `latest`로 가는지, CI와 Release workflow가 같은 Node·lockfile·전체 게이트를 쓰는지, `changeset status`가 배포 대상을 릴리스 계획에 잡는지 검사합니다.
