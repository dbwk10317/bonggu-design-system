...
export const ColorInput = forwardRef(
   ...
   */
  function ColorInput({ value, defaultValue = "#5CA8FF", presets = [], size = "md", fit = "flex", width, disabled, invalid, onChange, className, style, "aria-label": ariaLabel }, ref) {
  ...
  return (
    <div className={cx("bds-color", size === "sm" && "bds-color--sm", disabled && "bds-ctl--disabled", className)} style={frameStyle({ fit, width, style })}>
      <div className={cx("bds-ctl bds-color__ctl", size === "sm" && "bds-ctl--sm", (invalid ?? f?.invalid) && "bds-ctl--err")}>
        ...
        <input ref={ref} id={f?.id} aria-describedby={f?.describedBy} aria-label={f?.id ? undefined : `${ariaLabel ?? "색"} 16진값`} className="bds-mono" spellCheck={false} maxLength={7} value={text ?? v} disabled={disabled}
