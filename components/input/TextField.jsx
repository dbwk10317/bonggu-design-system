...
export const TextField = forwardRef(
   ...
   */
  function TextField({ size = "md", fit = "flex", width, prefix, suffix, icon, mono = false, invalid, disabled, className, style, ...rest }, ref) {
  ...
  return (
    <div className={cx("bds-ctl", size === "sm" && "bds-ctl--sm", (invalid ?? f?.invalid) && "bds-ctl--err", disabled && "bds-ctl--disabled", className)} style={frameStyle({ fit, width, style })}>
      ...
      <input ref={ref} id={f?.id} aria-describedby={f?.describedBy} aria-invalid={(invalid ?? f?.invalid) || undefined} required={f?.required} disabled={disabled} className={mono ? "bds-mono" : undefined} {...rest} />
