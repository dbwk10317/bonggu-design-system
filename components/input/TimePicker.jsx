...
export const TimePicker = forwardRef(
   ...
   */
  function TimePicker({ value = "", onChange, step = 5, size = "md", fit = "auto", width, disabled, className, style }, ref) {
  ...
  return (
    <div className={cx("bds-ctl", "bds-time", size === "sm" && "bds-ctl--sm", f?.invalid && "bds-ctl--err", disabled && "bds-ctl--disabled", className)} style={frameStyle({ fit, width, style })}>
      ...
      <select ref={ref} id={f?.id} aria-label="시" value={h ?? ""} disabled={disabled} onChange={(e) => set(Number(e.target.value), m)}><option value="" disabled>--</option>{Array.from({ length: 24 }, (_, i) => <option key={i} value={i}>{pad(i)}</option>)}</select>
