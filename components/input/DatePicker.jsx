...
export function DatePicker({ value, onChange, min, max, placeholder = "날짜 선택", size = "md", fit = "flex", width, disabled, className, style }) {
  ...
  return (
    <div ref={root} className={cx("bds-date", className)} style={frameStyle({ fit, width, style })}>
      <button type="button" id={f?.id} aria-describedby={f?.describedBy} aria-haspopup="dialog" aria-expanded={open} disabled={disabled} className={cx("bds-ctl", size === "sm" && "bds-ctl--sm", f?.invalid && "bds-ctl--err", disabled && "bds-ctl--disabled")} style={{ width: "100%", textAlign: "left" }} onClick={() => setOpen((o) => !o)}>
        <span className="bds-ctl__affix"><Icon name="calendar-blank" size={15} /></span>
        <span className={cx("bds-ellipsis", value && "bds-mono")} style={{ flex: 1, color: value ? undefined : "var(--text-3)" }}>{value ?? placeholder}</span>
      </button>
      {open && <div role="dialog" aria-label="날짜 선택" className="bds-date__pop">
        ...
        <div className="bds-cal__grid" role="grid">
          {DOW.map((d) => <span key={d} className="bds-cal__dow" role="columnheader">{d}</span>)}
          {cells.map((d) => { const s = iso(d); return <button key={s} type="button" role="gridcell" className={cx("bds-cal__d", d.getMonth() !== view.getMonth() && "bds-cal__d--out", s === today && "bds-cal__d--today")} aria-selected={s === value} disabled={!inRange(d)} onClick={() => { onChange?.(s); setOpen(false); }}>{d.getDate()}</button>; })}
        </div>
      </div>}
    </div>
  );
