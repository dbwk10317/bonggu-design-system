...
export const Select = forwardRef(
   ...
   */
  function Select({ options = [], placeholder, size = "md", fit = "flex", width, invalid, disabled, className, style, ...rest }, ref) {
  ...
  return (
    <div className={cx("bds-ctl bds-ctl--select", size === "sm" && "bds-ctl--sm", (invalid ?? f?.invalid) && "bds-ctl--err", disabled && "bds-ctl--disabled", className)} style={frameStyle({ fit, width, style })}>
      <select ref={ref} id={f?.id} aria-describedby={f?.describedBy} aria-invalid={(invalid ?? f?.invalid) || undefined} disabled={disabled} {...rest}>
