  ...
  return (
    <div ref={root} className={cx("bds-multi", "bds-combo", className)} style={frameStyle({ fit, width, style })}>
      <div className={cx("bds-ctl", f?.invalid && "bds-ctl--err", disabled && "bds-ctl--disabled")} onClick={() => input.current?.focus()}>
        ...
        <input ref={input} id={f?.id} aria-describedby={f?.describedBy} role="combobox" aria-expanded={open} aria-autocomplete="list" value={q} disabled={disabled || full} placeholder={value.length ? (full ? "" : "") : placeholder} onChange={(e) => { setQ(e.target.value); setOpen(true); }} onFocus={() => setOpen(true)} onKeyDown={onKey} />
        <Icon name="caret-down" size={13} className="bds-multi__caret" />
      </div>
      {open && !full && <ul role="listbox" className="bds-combo__list">
        {list.length === 0 && <li className="bds-combo__empty">{q ? "일치하는 항목 없음" : "모두 선택됨"}</li>}
        {list.map((o, i) => <li key={o.value} role="option" aria-selected={i === idx} className={cx("bds-combo__opt", i === idx && "bds-combo__opt--act")} onMouseEnter={() => setIdx(i)} onMouseDown={(e) => { e.preventDefault(); add(o.value); }}>{o.label}</li>)}
      </ul>}
    </div>
  );
}
