  ...
  return (
    <div ref={root} className={cx("bds-multi", "bds-combo", className)} style={frameStyle({ fit, width, style })}>
      <div className={cx("bds-ctl", f?.invalid && "bds-ctl--err", disabled && "bds-ctl--disabled")} onClick={() => input.current?.focus()}>
        ...
        <input ref={input} id={f?.id} aria-describedby={f?.describedBy} role="combobox" aria-expanded={open} aria-controls={`${uid}-list`} aria-activedescendant={open && !full && list[activeIdx] ? `${uid}-${activeIdx}` : undefined} aria-autocomplete="list" value={q} disabled={disabled || full} placeholder={value.length ? (full ? "" : "") : placeholder} onChange={(e) => { setQ(e.target.value); setOpen(true); }} onFocus={() => setOpen(true)} onKeyDown={onKey} />
