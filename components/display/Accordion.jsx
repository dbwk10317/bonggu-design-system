  ...
  return (
    <div className={cx("bds-acc", plain && "bds-acc--plain", className)} {...rest}>
      {items.map((it) => { const on = open.has(it.id); return <div key={it.id} className="bds-acc__item">
        <h3 style={{ margin: 0 }}><button type="button" className="bds-acc__hd" aria-expanded={on} aria-controls={uid + "-" + it.id} id={uid + "-h-" + it.id} onClick={() => toggle(it.id)}>{it.icon && <Icon name={it.icon} size={16} />}<span className="bds-ellipsis">{it.title}</span>{it.meta && <span className="bds-acc__meta">{it.meta}</span>}<Icon name="caret-down" size={14} className="bds-acc__chev" /></button></h3>
