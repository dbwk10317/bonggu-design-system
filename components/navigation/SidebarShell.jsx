...
export function SidebarShell({ brand, nav, navLabel = "주 메뉴", footer, topbar, statusbar, className, children, ...rest }) {
  ...
  return (
    <div className={cx("bds-shell", open && "bds-shell--open", className)} {...rest}>
      <aside id={id} className="bds-shell__side" aria-hidden={!open ? undefined : false}>
...
export function SidebarNavItem({ icon, label, href, target, active = false, badge, onClick }) {
  ...
  const inner = <>{icon && <Icon name={icon} />}<span className="bds-ellipsis" style={{ flex: 1 }}>{label}</span>{badge != null && <span className="bds-shell__item__badge">{badge}</span>}{target === "_blank" && <Icon name="arrow-square-out" size={12} />}</>;
