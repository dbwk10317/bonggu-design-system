...
export function ProgressBar({ value, label, detail, tone = "accent", size = "md", showValue = true, valueFormatter = (v) => `${Math.round(v * 100)}%`, fit = "flex", width, "aria-label": ariaLabel, className, style }) {
  ...
  return (
    <div className={cx("bds-progress", `bds-progress--${size}`, `bds-tone--${tone}`, !det && "bds-progress--indet", className)} style={frameStyle({ fit, width, style })}>
      {(label || (showValue && det)) && <div className="bds-progress__hd">{label && <span className="bds-progress__l">{label}</span>}{showValue && det && <span className="bds-progress__v bds-mono">{valueFormatter(v)}</span>}</div>}
      <div className="bds-progress__track" role="progressbar" aria-label={ariaLabel ?? (typeof label === "string" ? label : undefined)} aria-valuemin={0} aria-valuemax={100} aria-valuenow={det ? Math.round(v * 100) : undefined}>
