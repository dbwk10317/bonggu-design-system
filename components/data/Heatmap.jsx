    ...
    <div role="img" aria-label={ariaLabel} aria-describedby={srId} className={cx("bds-heat", className)} style={frameStyle({ fit, width, style: { "--cell": `${cell}px`, "--gap": `${gap}px`, "--cols": cols.length, ...style } })}>
      <div className="bds-heat__grid" tabIndex={0} onKeyDown={onKey} onBlur={() => setHover(null)}>
        ...
        {rows.map((r, i) => <React.Fragment key={i}>
          ...
          {cols.map((_, j) => { const v = values[i]?.[j]; return <i key={j} className={cx("bds-heat__cell", v == null && "bds-heat__cell--na", hover && hover[0] === i && hover[1] === j && "bds-heat__cell--on")} data-step={step(v)} onMouseEnter={() => setHover([i, j])} onMouseLeave={() => setHover(null)} />; })}
        </React.Fragment>)}
      </div>
      ...
      <div className="bds-heat__foot">
        <span className="bds-heat__tip" role="status">{hover && hover[1] >= 0 ? <><b>{rl(rows[hover[0]])} · {cl(cols[hover[1]])}</b> <span className="bds-mono">{fmt(hover[0], hover[1])}</span></> : "\u00a0"}</span>
