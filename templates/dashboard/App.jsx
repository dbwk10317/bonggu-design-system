  ...
  return (
    <PageStack aria-label="쿨러 LCD">
      ...
      <Panel padding="sm"><Tabs aria-label="쿨러 콘텐츠" value={tab} onChange={setTab} items={[{ value: "presets", label: "프리셋", count: 3 }, { value: "photos", label: "사진", count: 0 }]} />
        {tab === "presets" ? (
          <Grid min={200}>{presets.map(([n, d], i) => <Panel key={n} padding="sm" interactive selected={i === 0}><div className="kit-lcd" aria-hidden="true"><span>BONGGU</span><b>51°</b><small>CPU 12% · GPU 8%</small></div><div><b style={{ fontSize: 12.5, fontWeight: 600 }}>{n}</b><p className="kit-dim" style={{ fontSize: 11 }}>{d}</p></div><div className="kit-actions"><Button size="sm" variant={i === 0 ? "secondary" : "primary"} fit="flex" disabled={i === 0}>{i === 0 ? "적용 중" : "적용"}</Button></div>{i === 0 && <Tag accent className="kit-corner">적용 중</Tag>}</Panel>)}</Grid>
        ) : (
