# 봉구 대시보드 디자인 시스템 · 규칙 (RULE)

이 문서가 디자인 규칙의 유일한 출처다. 시각·카피 규범, 결측·fit·접근성 계약, 동작 계약, 컴포넌트 목록과 선택 가이드, 공개 API·버전·배포 계약, 릴리스 절차를 담는다. 기계로 판별할 수 있는 항목의 검사는 `tests/rule-regressions.cjs`에 있다. 작업 원칙은 [AGENTS.md](AGENTS.md), 검증 실행법은 [tests/README.md](tests/README.md), 프로젝트 소개와 설치는 [README.md](README.md)([한국어](README_KO.md))를 본다.

운영·모니터링 대시보드를 위한 디자인 시스템이다. 실시간 수치, 장치 제어, 사용자·리소스 관리처럼 **숫자가 많고 상태가 자주 바뀌는 화면**을 라이트·다크, PC·태블릿·모바일 어디서나 같은 규칙으로 그린다. 시각 규칙은 `봉구서버 스타일가이드 v1`을 토큰으로 옮긴 것이고, 마스코트 **봉구**(시츄+페키니즈 믹스)가 시스템의 감정 표현을 맡는다.
...
코드 주석이나 컴포넌트 문서에 여기 없는 규칙을 새로 만들지 않는다.
...
- **자연 높이의 요약은 내용만큼 성장한다.** 단일 측정값의 도형·값·설명·눈금은 줄바꿈과 자연 높이로 배치하며 스크롤 컨테이너로 만들지 않는다. 명시적으로 높이를 제한한 영역은 읽을거리를 보존하는 내부 탐색을 제공한다. 도형 좌표와 텍스트 측정에는 같은 CSS 좌표계를 사용하며 화면 확대·변환 배율을 중복 적용하지 않는다.
- **사건 목록의 시간과 본문은 서로의 영역을 침범하지 않는다.** 시간 열은 실제 문자열 길이와 남은 본문 공간을 함께 고려하고, 긴 날짜·식별자는 각자 배정된 영역 안에서 줄바꿈한다. 연결선은 글자 길이에 관계없이 사건 표시를 따라간다.
...
- `README.md`·`README_KO.md` · 프로젝트 소개, 설치, 빠른 시작, 문서 안내. 규칙을 적지 않는다
- `RULE.md` · 이 문서. 규칙의 유일한 출처
- `docs/screenshots/` · README의 예시 화면. `templates/dashboard`를 1280·390 폭, 라이트·다크로 캡처한 PNG. 배포물에 들어가지 않는다
...
**발행은 태그다.** 버전 확정과 태그 생성은 로컬에서 하고, `.github/workflows/release.yml`은 태그 푸시에서 게이트(`npm test`)를 돌린 뒤 올리기만 한다. `main` 푸시로는 아무것도 발행되지 않고, 버전 PR도 쓰지 않는다.

```bash
npx changeset                                     # 변경마다 소비자 관점의 설명과 SemVer 영향도를 남긴다
...
- 워크플로는 올리기 전에 둘을 막는다. 게이트를 통과하지 못하면 올리지 않고, 태그와 `package.json`의 버전이 다르면 올리지 않는다. 채널은 버전이 정한다. prerelease(`-`가 있는 버전)는 `next`, 정식은 `latest`.
- 인증은 Trusted Publishing(GitHub Actions OIDC)이다. 워크플로는 `id-token: write`만 갖고 토큰을 읽지 않으며, `setup-node`에 `registry-url`을 주지 않는다(주면 `NODE_AUTH_TOKEN`을 읽는 `.npmrc` 줄이 생겨 OIDC 경로와 충돌한다). npm 11.5.1 이상이 필요해 워크플로가 먼저 확인한다. npmjs 패키지 설정의 Trusted Publisher에 저장소 `dbwk10317/bonggu-design-system`과 워크플로 파일 이름 `release.yml`(environment 공란)이 등록돼 있어야 한다. provenance는 자동으로 붙는다.
- Publish 단계는 verbose로 돌아 id-token 획득, 교환 요청(`/-/npm/v1/oidc/token/exchange/package/…`)과 응답 코드가 로그에 남는다. 교환 요청 줄 없이 `ENEEDAUTH`면 npmjs 쪽 Trusted Publisher 등록부터 본다. 인증 문제로 Publish가 죽어도 태그를 다시 만들 필요는 없다. 원인을 고친 뒤 `gh run rerun <run-id> --failed`로 같은 실행을 재개한다. 워크플로 파일 자체를 고쳐야 하고 아직 아무것도 발행되지 않았다면 태그를 새 커밋으로 옮긴다.
- 발행 결과는 `npm view @dbwk10317/bonggu-design-system versions`로 확인한다. 레지스트리 루트 문서는 몇 분 늦게 나타나고 버전별 문서(`/<버전>`)가 먼저 200이 된다. 이미 배포된 버전은 덮어쓰지도 지우지도 못하므로 잘못 나간 버전은 `npm deprecate`로 표시하고 수정 버전을 낸다.
- `changeset status`는 `main` 브랜치와의 diff로 동작한다. 태그 체크아웃처럼 `main`이 없는 곳에서는 실행되지 않으므로 `tests/release-regressions.cjs`가 그 경우를 건너뛴다고 적고 지나간다.
- 루트 `package.json`에 `workspaces`를 선언하지 않는다. 선언하면 changesets가 루트를 versionable 목록에서 빼 릴리스 계획을 세우지 못한다.
- 초기 범위에서 컴포넌트별 패키지 분리, 토큰의 독립 버전, CommonJS 이중 배포, 별도 문서 플랫폼은 제외한다. 실제 소비 요구가 생기면 범위를 다시 정한다.
