...
function Gauge({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: cx("bds-gauge", "bds-gauge--" + t, frame.height != null && "bds-gauge--bounded", className),
    style: frame,
    role: "meter",
    "aria-valuemin": 0,
    "aria-valuemax": max,
...
function Cartesian({
  ...
  const pointerX = (/** @type {SVGSVGElement} */svg, /** @type {number} */clientX) => {
    const rect = svg.getBoundingClientRect();
    return (clientX - rect.left) * w / rect.width;
  };
  const onMove = (/** @type {import("react").MouseEvent<SVGSVGElement> | import("react").TouchEvent<SVGSVGElement>} */e) => {
    setHover(indexAt(pointerX(e.currentTarget, "clientX" in e ? e.clientX : e.touches[0]?.clientX ?? 0)));
  };
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("svg", {
    ...
    onPointerDown: e => {
      ...
      e.preventDefault();
      const i = indexAt(pointerX(e.currentTarget, e.clientX));
      setDrag(i);
      setHover(i);
    ...
    onPointerUp: e => {
      ...
      const end = indexAt(pointerX(e.currentTarget, e.clientX));
...
function FilterBarImpl({
  ...
  const add = () => {
    ...
    } while (filters.some(token => token.id === id));
...
// components/layout/JustifiedGallery.jsx
try { (() => {
const { useEffect, useMemo, useRef, useState } = React;
const { cx, frameStyle, numeric } = __ds_scope;
...
function justifyRows(items, width, target, gap) {
  if (!(width > 0) || !items.length) return [];
  const ratios = items.map(item => {
    const w = numeric(item.width),
      h = numeric(item.height);
...
function JustifiedGallery({
  ...
  useEffect(() => {
    ...
    const measure = () => {
      const w = el.clientWidth,
        gap = parseFloat(getComputedStyle(el).rowGap) || 0;
      setBox(p => p.w === w && p.gap === gap ? p : {
        w,
        gap
      });
    };
  ...
  return /*#__PURE__*/React.createElement("div", {
    ref: ref,
    role: "group",
    className: cx("bds-jgal", className),
    style: frameStyle({
      fit,
      width,
      style
    ...
    key: row.tiles[0].index,
    className: "bds-jgal__row"
  }, row.tiles.map(({
    index,
    width: w
  }) => {
    ...
    return onOpen ? /*#__PURE__*/React.createElement("button", {
      key: item.id,
      type: "button",
      className: "bds-jgal__tile",
      style: size,
      "aria-label": `${item.alt} 상세보기`,
    ...
    className: "bds-jgal__foot"
  }, footer));
}
Object.assign(__ds_scope, { justifyRows, JustifiedGallery });
})(); } catch (e) { __ds_ns.__errors.push({ path: "components/layout/JustifiedGallery.jsx", error: String((e && e.message) || e) }); }
...
function TopNav({
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, skipTo && /*#__PURE__*/React.createElement("a", {
    className: "bds-skip",
    href: `#${skipTo}`
  }, "본문으로 건너뛰기"), /*#__PURE__*/React.createElement("header", {
...
__ds_ns.JustifiedGallery = __ds_scope.JustifiedGallery;
